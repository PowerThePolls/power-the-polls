...
async function getPartnerReportList() {
   ...
   let reportList = [queryResponse.objects, dashboardResponse.objects];
   while (queryResponse.meta.next || dashboardResponse.meta.next) {
      queryResponse = await callActionKit(queryResponse.meta.next);
      dashboardResponse = await callActionKit(dashboardResponse.meta.next);
      reportList = reportList.concat(queryResponse.objects);
      reportList = reportList.concat(dashboardResponse.objects);
   }
   return reportList;
}
